...
class AccountControllerTest {
  ...
  public void testAllAccountsRequest() throws Exception {
    ...
    mockmvc.perform(post("/account")
        ...
        //.param(testAccount)
        .contentType(MediaType.APPLICATION_JSON) // dlaczego ma być JSON? - bo REST
    );
    mockmvc.perform(post(
        "/account") // czy jest sposob wyslac w 1 requescie, stworzyc taki czy nie ma sensu? - stworzyc metode przyjmujaca liste kont i je tworzaca
        .content(json(testAccount2))
        //.param(testAccount)
        .contentType(MediaType.APPLICATION_JSON)
    );

    mockmvc.perform(get("/accounts")) // DLACZEGO PODPOWIEDZIAL REQUEST BUILDER???
        .andExpect(status().isOk())
        //.andExpect(content().contentType(MediaType.APPLICATION_JSON))
        //.andExpect(jsonPath("$").isEmpty())
        .andExpect(jsonPath("$[0].description", is(testAccount.getDescription())))
        .andExpect(jsonPath("$[1].description", is(testAccount2.getDescription()))) //czy musi byc id gdy tylko 1 obietk znajduje sie w mapie?
  ...
  public void testDeleteAccount() throws Exception {
    ...
    mockmvc.perform(delete("/account/" + response)); // logger z info by wypadalo zaimplementowac?
